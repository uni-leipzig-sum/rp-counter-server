...
const d_pin_direction_t DpinDirs[] = {
  {"IN", 0},
  {"OUT", 1},
  NULL
};

static int32_t name_to_id(pin_to_id_map_t *map, const char *name)
{
  while(*map != NULL) {
...
int Analog_GetPinValue(int argc, char **argv, char **res, size_t *resLen)
{
  ...
  *resLen = safe_printf(res, "%g", value);
	if (*resLen < 0) {
		// Error
...
int Digital_GetPinState(int argc, char **argv, char **res, size_t *resLen)
{
  ...
  *resLen = safe_printf(res, "%u", state);
	if (*resLen < 0) {
		// Error
		RP_LOG(LOG_ERR, "*unknown error.\n");
		*resLen = 0;
...
int Digital_GetPinDirection(int argc, char **argv, char **res, size_t *resLen)
{
  ...
  *resLen = safe_printf(res, "%s", dir_s);
	if (*resLen < 0) {
		// Error
		RP_LOG(LOG_ERR, "*unknown error.\n");
		*resLen = 0;
